...
**Phase 4 — Evals, UI, runners** 🔶 *eval harness, eval gate, learning governance, and the admin UI shipped 2026-08-02 (UI note: session-cookie auth against our own users table instead of fastapi-users — we already owned the rows and pwdlib hashing; OIDC stays planned as additive). Also shipped beyond plan: retention crons, per-tenant concurrency caps, callback/error-rate alerting, job listing + dead-letter retry, CI. Remaining: OIDC, version aliases, sandboxed code runners + code graders*

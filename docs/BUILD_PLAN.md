...
## Binary upload screening (designed, not built — audit-4 #6 residual)

**This is not a new hook.** The injection pre-hook already exists, is default-on, and is correct; what is missing is anything feeding it. `runtime/runner._load_file_content` appends a file to the screened `texts` list only when its content type starts with `TEXT_CONTENT_TYPES`; everything else becomes opaque `BinaryContent` and goes straight to the model. So a genuine PDF carrying injection text in a compressed content stream reaches `screen_untrusted()` as *nothing at all* — the hook is handed an empty feed and matches nothing, while the model extracts the text and reads it. `sniff_content_type` closes the adjacent hole (text *posing* as a PDF) and cannot close this one: here the bytes really are a PDF.

The fix is an extraction step upstream of the pre-hook. `screen_untrusted()`, both tiers, tenant-defined rules, and the shared `INJECTION_SCREEN_TIMEOUT_S` budget stay exactly as they are.

- **Extract, don't intercept.** A `runtime/extract.py` registry keyed by media type (`application/pdf` → pypdf, OOXML → python-docx / openpyxl, images → OCR only if scans are judged worth the dependency). The model still receives the real `BinaryContent`: extraction does not replace its input, it gives the screen a view of the same text the model will read. One branch in `_load_file_content` changes, and everything downstream composes unchanged.
- **Bounded, and off the event loop.** Uploads cap at 50 MiB, a decompression bomb inside a PDF is far larger, and extraction is CPU-blocking. It runs via `anyio.to_thread` with its own wall-clock cap and an output cap applied *before* the extracted text joins the screen's shared budget. This is the ReDoS lesson applied verbatim: that finding's real bite was that blocking happened on the worker's **event loop**, stalling every concurrent job in the process rather than one.
- **The parsers are themselves new attack surface.** This means running pypdf over hostile attacker-supplied input inside the worker — the process holding `SECRET_KEY` and every decrypted provider credential. Un-contained, that is a worse trade than the problem it solves. Containment already exists: the `docker` backend in `runtime/runners.py` gives throwaway containers with no network, dropped capabilities, and memory/CPU/wall-clock caps — a better fit for hostile-document parsing than the eval code graders it was built for. The catch is that it sits behind `RUNNER_BACKENDS` and needs the Docker socket, so the sandboxed path cannot be the default, which feeds directly into the open question below.
- **Coverage is inherently partial.** Encrypted PDFs, scans without OCR, and formats with no extractor will always exist. Any design that claims to close this hole completely is wrong about extraction.

**Open policy call — the actual threat-model decision this item was parked for.** What happens when a binary cannot be read?

1. *Fail open* (screen what extracts, pass the rest): breaks no workflow, but the bypass survives for anything unparseable. An honest improvement, not a closed hole.
2. *Fail closed* (reject unextractable binaries): genuinely closes it; breaks every image and scanned-document workflow.
3. *Conditional on grants* — fail closed only for agents holding sensitive grants (MCP tool grants, `rw` data-store grants, delegation), fail open otherwise. **Recommended.** Injected text matters because the agent can *act* on it, so a read-only summarizer and an agent with write grants deserve different answers. It is expressible from `version.tool_grants` / `data_store_grants`, already loaded a few lines above the screen.

**Suggested phasing.** Ship extraction fail-open (pypdf + python-docx) gated per-tenant like the classifier tier — a contained change to one branch that makes the common case, a text-bearing PDF, screened. Add the grants-conditional refusal as a separate decision once extraction coverage is observable in practice. OCR last or never: heaviest dependency, least certain return.
...
2. **Disposable containers (when agents need real filesystems/packages)** ✅ *tier 2 shipped 2026-08-03 as the `docker` backend of `runtime/runners.py` — a `Runner` protocol (run/validate) + registry that formalizes "pluggable behind the same runner interface": `monty` stays the default, operators opt in via `RUNNER_BACKENDS` (comma-separated, first entry is default, needs the Docker socket mounted into api/worker — commented lines in compose), and eval code checks pick per-check via an optional `"runner"` field validated on write. One hardened throwaway container per call (`python:3.12-slim` by default, configurable): real CPython + stdlib/packages, network disabled, all capabilities dropped, no-new-privileges, runs as nobody, pids/memory capped (no swap), 1 CPU, wall-clock enforced from outside (kill + force-remove). Request enters as JSON in an env var, verdict leaves as one JSON line on stdout. **srt evaluated and passed over for tier 2**: Anthropic's sandbox-runtime restricts a process with OS primitives (Seatbelt on macOS, bubblewrap on Linux), but our worker itself runs in a compose container where bubblewrap needs user namespaces that are typically unavailable, and Seatbelt only applies to native-macOS deployments — it could return later as a native-host backend. gVisor hardening still deferred until multi-tenant-hostile.*
3. **Hosted sandboxes (if we don't want the infra)** — E2B, Modal, and kin slot into the same `REGISTRY` as tier-3 backends when someone needs VM-grade isolation without local infra; not shipped, since they'd make an external account a dependency of an OSS install. **E2B-as-the-abstraction was considered and rejected 2026-08-03**: its sandboxes are Firecracker microVMs needing KVM, so "local E2B" means operating their whole self-host stack (Terraform + Nomad + Consul, GCP-first) — it cannot ship inside our compose file, and the SDK is coupled to their control plane. The abstraction stays ours (two functions); E2B is just a backend. Daytona ruled out (closed-sourced June 2026).

...
**Two smaller things the parity claim above did not cover**, both closed
2026-08-25. Neither was a missing route — one was a field with no meaning and
one was a shape the list page could not express:

- [x] ~~**`EventSourceCreate.config`** is not on the event-source form.~~ The
  column is gone instead. It was written by
  `POST /v1/tenants/{id}/event-sources` and read by nothing — a source is
  resolved at ingest entirely through `payload_template`, `dedup_key_path` and
  `secret_hash` — so the choice was to invent a meaning for it or delete it,
  and inventing one would have been building a feature to justify a column.
  Now that the repo is public, an inert field that round-trips through
  `EventSourceOut` is something people can start storing values in, which
  turns dead weight into a compatibility obligation. Dropped in migration
  `d3f5a91c07be` (both dev rows held `{}`, so nothing was lost). The plausible
  future need is per-source webhook signature verification; when that is built
  it gets named, validated columns like its two siblings.
- [x] ~~**Revoking one specific key of *another* user.**~~ Each user now has
  `/ui/users/{id}/keys`, reached from a **Keys** button on the list. The
  original objection — that a page showing only a count makes picking the
  right row guesswork — was about the *list*, and it still stands: the list
  keeps the count and the all-at-once **Revoke keys** button, because from
  there the question is "cut this person off", and that should not be a
  row-by-row chore that can be left half-done. Picking one key is a different
  question, so it got a page whose subject is one person, listing name, **key
  id** and issue time. The id is what makes rows distinguishable at all — the
  secret is not stored, `name` is optional, and several keys can share a day —
  and it is the handle `DELETE /v1/users/{id}/keys/{key_id}` itself takes. The
  revoke route is scoped by `user_id` as well as `key_id`, so posting someone
  else's key id under this user's path does nothing, matching the API.
...
## Schema: migrations vs. models

The suite builds its schema with `Base.metadata.create_all`, not Alembic
(`tests/conftest.py`). That is deliberate — it is fast, it needs no migration
before a model change can be run, and it builds from the metadata the queries
are actually written against — but it means CI's `test` check passes whether or
not the migration chain is right.

They had already drifted, found 2026-08-25 while checking whether a new
migration was covered by any test. `ck_memory_versions_status_valid`
(`models.py`, `MemoryVersion.__table_args__`) existed in every `create_all`
database and in **no** migrated one, including the local dev DB: `a420524ae0f1`
added `memory_versions.status` with an autogenerated `op.add_column`, and
**Alembic's autogenerate does not emit CHECK constraints**. So tests rejected an
out-of-range status and a deployment would have accepted it. Added in
`b4a1f2e83c67`. A full catalog comparison found nothing else wrong: all 166
columns and every index matched, 79 constraints vs 80.

`alembic check` does not cover this — it reports "No new upgrade operations
detected" against the drifted schema, for the same autogenerate reason. So
`tests/test_migrations.py` builds a scratch database each way and diffs columns,
constraints and indexes (ordering excluded: `add_column` appends where
`create_all` uses model order, which nothing can observe). A second test walks
`upgrade head` then `downgrade base` and asserts nothing but `alembic_version`
survives — a failed downgrade rolls back, so it leaves a consistent database and
a sane `alembic current`, and shows up only in stderr. This branch shipped
exactly that bug: a `drop_constraint` given the fully-qualified name, which the
naming convention then expanded a second time. Both tests were confirmed to
fail on the bug they target before being kept. ~2.7s, once per run.

Switching the suite itself to Alembic was considered and rejected: it would
stop tests building from the models the queries target, require a migration
before any model change could be run, and force the whole chain to keep
replaying forever — while still not exercising the risky part, which is
migrations against *existing data*, not DDL on an empty database.
